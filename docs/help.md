...
Scan codes: hexadecial codes or code sequences separated by blank space.

# Linux XKB Key Codes

Click on the key to see the notation used in the XKB keyboard layouts, <code>/usr/share/X11/xkb/symbols</code>.
Shown without the angilar brackets.

# Windows Virtual Keys

`VK_*` key names and hexadecimal values.
Click on the key to see a full name if it is abbreviated.
